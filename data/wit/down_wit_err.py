...
def req_imgs(url_info):
    ...
    try:
        ...
        assert file_ext in {"png", "jpeg", "svg", "gif"}
...
if __name__ == '__main__':
    ...
    url_list = {samp[0]:[samp[0], samp[3]] for samp in samples}
    
    url_set = set(url_list.keys())
    did_set = set(l.split('.')[0].split('/')[-1] for l in glob.glob('/home/shared/dataset/wit/img/retry/*'))
    
    urls = url_set - did_set
    print("retry number is", len(urls))
    down_imgs([url_list[u] for u in urls])
